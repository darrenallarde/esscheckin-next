...
export function OrganizationProvider({ children }: OrganizationProviderProps) {
  ...
  const orgSlugFromPath =
    (params.org as string | undefined) || extractOrgSlugFromPath(pathname);
  ...
  const fetchOrganizations = React.useCallback(async () => {
    try {
      ...
      const { data: orgs, error } = await supabase.rpc(
      ...
      const mappedOrgs: Organization[] = orgs.map(
        (org: {
      ...
      if (orgSlugFromPath) {
        selectedOrg =
          mappedOrgs.find((o) => o.slug === orgSlugFromPath) || null;
        const orgData = orgs.find(
          (o: { organization_slug: string }) =>
            o.organization_slug === orgSlugFromPath,
        );
