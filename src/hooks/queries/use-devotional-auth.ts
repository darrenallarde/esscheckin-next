...
export function useDevotionalAuth() {
  ...
  const verifyEmailOtp = useCallback(async (email: string, code: string): Promise<LinkResult | null> => {
    setIsLoading(true);
    setError(null);
    try {
      const supabase = createClient();
      ...
      // Link profile
      const { data: linkData, error: linkError } = await supabase.rpc("link_email_to_profile", {
        p_email: email,
      });
      if (linkError) {
        setError(linkError.message);
  ...
  const findProfileForSignup = useCallback(async (orgId: string, identifier: string): Promise<FindProfileResult | null> => {
    setIsLoading(true);
    setError(null);
    try {
      const supabase = createClient();
      const { data, error: rpcError } = await supabase.rpc("find_profile_for_signup", {
        p_org_id: orgId,
      ...
      setError(err instanceof Error ? err.message : "Failed to look up profile");
      return null;
    } finally {
      setIsLoading(false);
    }
  ...
  const checkUsername = useCallback(async (orgId: string, username: string): Promise<boolean> => {
    try {
      const supabase = createClient();
  ...
  const signUpWithUsername = useCallback(async (
    ...
    try {
      const response = await fetch("/api/devotional/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ org_id: orgId, org_slug: orgSlug, profile_id: profileId, username, password }),
      });
      const result = await response.json();
      if (!response.ok || !result.success) {
