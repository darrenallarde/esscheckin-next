-- Add extended student fields to pastoral analytics
-- Includes instagram_handle, mother/father details for quick pastoral action
...
RETURNS TABLE(
  ...
  -- Extended fields for quick action
...
BEGIN
  ...
  ORDER BY action_priority DESC, s.first_name, s.last_name;
END;
$$;

COMMENT ON FUNCTION public.get_pastoral_analytics IS
'Calculates student belonging status with extended fields for quick pastoral action. Includes instagram_handle, mother/father details.';
