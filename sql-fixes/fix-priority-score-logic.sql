-- Fix priority score logic in pastoral analytics function
-- ISSUE: Priority scores were inverted (Missing=6, Ultra-Core=1) but sorted ascending
-- RESULT: Ultra-Core students appeared first instead of Missing students
-- FIX: Invert the scores so higher number = higher priority, and rename to match TypeScript interface
...
RETURNS TABLE(
  ...
  action_message text,
  action_priority integer  -- RENAMED from priority_score to match TypeScript interface
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      -- Core: 4+ check-ins in 8 weeks (roughly 1x/week)
    ...
    -- FIXED: Inverted logic so ascending sort (a - b) shows most urgent first
    CASE
      WHEN sp.last_checkin IS NULL OR sp.last_checkin < v_sixty_days_ago THEN 1  -- Missing (most urgent)
      WHEN sp.last_checkin < v_thirty_days_ago THEN 2  -- On Fringe
      WHEN sp.checkins_last_4weeks < sp.checkins_prev_4weeks AND sp.checkins_last_4weeks < 2 THEN 3  -- Declining
      WHEN sp.checkins_8weeks >= 2 AND sp.checkins_8weeks < 4 THEN 4  -- Connected
      WHEN sp.checkins_8weeks >= 4 THEN 5  -- Core
      WHEN sp.checkins_last_4weeks >= 5 THEN 6  -- Ultra-Core (least urgent)
      ELSE 4  -- Default to Connected priority
...
'Calculates student belonging status with updated thresholds: Ultra-Core (5+ in 4 weeks), Core (4+ in 8 weeks), Connected (2-3 in 8 weeks), On Fringe (30-60 days), Missing (60+ days). Returns action_priority where 1=most urgent (Missing), 6=least urgent (Ultra-Core).';
