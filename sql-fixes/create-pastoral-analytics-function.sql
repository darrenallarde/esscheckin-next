-- Create function to calculate student belonging status and pastoral analytics
-- This analyzes attendance patterns over the last 8 weeks
  ...
  student_metrics AS (
    SELECT
      *,
      ...
      CASE
        -- Ultra-Core: 2x/week (both Wednesday AND Sunday) - at least 12 check-ins in 8 weeks
        WHEN checkins_8weeks >= 12 AND wednesday_checkins >= 4 AND sunday_checkins >= 4 THEN 'Ultra-Core'

        -- Core: 1x/week consistently - at least 6 check-ins in 8 weeks
        WHEN checkins_8weeks >= 6 THEN 'Core'

        -- Connected: 2x/month - at least 3 check-ins in 8 weeks
        WHEN checkins_8weeks >= 3 THEN 'Connected'

        -- On the Fringe: Haven't seen in 30 days but within 60 days
        WHEN last_checkin IS NOT NULL AND last_checkin < v_thirty_days_ago AND last_checkin >= v_sixty_days_ago THEN 'On the Fringe'

        -- Missing: Haven't seen in 60+ days or never
        ELSE 'Missing'
...
-- Add comment
COMMENT ON FUNCTION public.get_pastoral_analytics IS 'Calculates student belonging status and generates pastoral recommendations based on 8-week attendance patterns';
