-- =============================================================================
-- ADD ICON COLUMN TO ORGANIZATIONS + UPDATE get_user_organizations RPC
-- =============================================================================
-- Allows organizations to choose a custom sidebar icon (lucide-react icon name).
-- NULL = default Dog icon.
-- =============================================================================
...
AS $$
  ...
  WHERE p.user_id = p_user_id
    ...
    AND o.status = 'active'
    AND om.role IN ('owner', 'admin', 'leader', 'viewer')

  UNION

  -- Fallback: Also check legacy organization_members table
  SELECT
    o.id AS organization_id,
    o.name AS organization_name,
    o.slug AS organization_slug,
...
Returns all organization settings including icon for the frontend.';
