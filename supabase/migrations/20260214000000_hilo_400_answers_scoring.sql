-- Hi-Lo: Update scoring formula for 400-answer games.
-- HIGH rounds: (401 - rank) * multiplier (was 201)
-- LOW rounds: rank * multiplier (unchanged — lower rank = rarer = more points)
...
BEGIN
  ...
  -- Calculate score (updated for 400-answer games)
  IF NOT v_on_list THEN
    v_score := 0;
  ELSIF v_direction = 'high' THEN
    v_score := (401 - v_rank) * v_multiplier;
  ELSE
