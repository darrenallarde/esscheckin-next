-- Remove the dangerous public policies that expose all student data
DROP POLICY IF EXISTS "Public can search students for check-in" ON public.students;
DROP POLICY IF EXISTS "Public can create check-ins" ON public.check_ins;
DROP POLICY IF EXISTS "Public can register new students" ON public.students;

-- Create a secure function for check-in searches that only returns minimal, safe data
...
-- Create a secure function for student registration during check-in
...
-- Create a secure function for check-in confirmation
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT FALSE, 'Student not found', ''::TEXT, ''::TEXT;
    RETURN;
  END IF;
  
  -- Create check-in record
  INSERT INTO public.check_ins (student_id) VALUES (p_student_id);
  
  RETURN QUERY SELECT TRUE, 'Check-in successful', student_record.first_name, student_record.user_type;
END;
$$;

-- Grant execute permissions on the functions to anonymous users
...
-- Keep admin access policies intact for authenticated admin users
-- (existing admin policies remain for full data access when authenticated)