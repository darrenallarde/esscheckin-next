-- =============================================================================
-- Co-Pilot V2: AI-as-Brain Architecture
-- Replaces hardcoded scoring with raw data gathering for Claude to reason over
-- =============================================================================

-- 1. Expand copilot_drafts cache table for full briefing storage
ALTER TABLE copilot_drafts
  ADD COLUMN IF NOT EXISTS briefing_json JSONB,
  ADD COLUMN IF NOT EXISTS model_used TEXT;

-- Make draft_message nullable (V2 stores full briefing in briefing_json instead)
ALTER TABLE copilot_drafts
  ALTER COLUMN draft_message DROP NOT NULL;

-- 2. New data-gathering RPC (no scoring — Claude does the thinking)
-- Keeps old get_copilot_briefing for rollback safety
CREATE OR REPLACE FUNCTION get_copilot_briefing_v2(
  p_org_id UUID,
...
BEGIN
  ...
  combined AS (
    SELECT
      ...
      -- Simple relevance score for initial filtering (NOT for ranking — Claude ranks)
      (
  ...
  FROM combined c
  WHERE c.relevance >= 1  -- Anyone with at least one signal worth evaluating
  ORDER BY c.relevance DESC, c.days_absent DESC
  LIMIT p_limit;
END;
$$;

GRANT EXECUTE ON FUNCTION get_copilot_briefing_v2(UUID, INT) TO authenticated;

COMMENT ON FUNCTION get_copilot_briefing_v2 IS
  'V2: Returns raw student data with signals for AI co-pilot reasoning. No scoring — Claude ranks and prioritizes. Filters by organization_id.';
