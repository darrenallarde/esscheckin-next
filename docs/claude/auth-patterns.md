...
| Service Role | Created in API routes with `SUPABASE_SERVICE_ROLE_KEY` | `send-phone-otp`, `verify-phone-otp` | Bypasses RLS entirely                 |
...
Stored in `auth.users.email`. Phone OTP also sets `auth.users.phone`.

> **Note:** Username/password auth (`{username}@{org_slug}.sheepdoggo.app`) was removed in Feb 2026. Existing accounts with that pattern still work via phone/email OTP.
